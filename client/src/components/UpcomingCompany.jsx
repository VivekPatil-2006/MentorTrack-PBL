...
function UpcomingCompany({ email }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/upcoming-companies', {
        ...
        body: JSON.stringify({ companyName, criteria, type, date, description, email })
      });
      const data = await res.json();
      if (res.ok) {
        setMessage(data.message);
        setCompanyName(''); setCriteria(''); setType(''); setDate(''); setDescription('');
      } else {
  ...
  return (
    <div style={styles.formCard}>
      <h3 style={styles.title}>Add Upcoming Company</h3>
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <input type="text" placeholder="Criteria" value={criteria} onChange={e => setCriteria(e.target.value)} style={styles.input} />
        <input type="text" placeholder="Type (e.g., UG/PG/PPO)" value={type} onChange={e => setType(e.target.value)} style={styles.input} />
